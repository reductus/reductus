...
def load_metadata(filename, file_obj=None):
    ...
    """
    return load_entries(filename, file_obj=file_obj, meta_only=True)


def load_entries(filename, file_obj=None, entries=None, meta_only=False):
    """
    Load the summary info for all entries in a NeXus file.
    """
    ...
    for name, entry in file.items():
        ...
        if entry.attrs.get('NX_class', None) == 'NXentry':
            if entry['DAS_logs/experiment/instrument'].value == "CANDOR":
                data = NCNRNeXusCANDOR(entry, name, filename)
            else:
...
class NCNRNeXusCANDOR(refldata.ReflData):
    """
    NeXus reflectometry entry for CANDOR.
