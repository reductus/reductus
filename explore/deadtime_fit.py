...
def attenuation_estimate(observed_rate, tau_NP, tau_P, above=False):
    """
    ...
    *tau_NP* and *tau_P* can be estimated from pairs of rate measurements,
    ...
    Returns the scale factor *A* for each observed rate such that *A*
    times the observed rate is the incident rate.
...
def deadtime_estimate(attenuated, unattenuated, mode='auto'):
    ...
    *attenuator* and *incident_rate*.  The incident rate is a pair of
    ...
    if mode == 'auto':
        ...
        # Pick single vs combined subject to reduction in chisq being
...
def _show_inversion(wo, tau_P_f, tau_NP_f):
    ...
    Ao = lambda R,dR: attenuation_estimate(([R],[dR]), tau_NP_f, tau_P_f)[0].n
