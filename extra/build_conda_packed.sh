...
case $OSTYPE in 
  darwin*) cp -r ./extra/platform_scripts/reductus.app "$pkgdir" ;;
  msys*) cp ./extra/platform_scripts/reductus.bat "$pkgdir" ;;
  linux*) cp -r ./extra/platform_scripts/make_linux_desktop_shortcut.sh "$pkgdir" ;
          cp -r ./extra/platform_scripts/reductus.sh "$pkgdir" ;;
esac
...
cp ./reductus/web_gui/webreduce/img/*.svg $pkgdir/share/icons
#cp $SCRIPT_DIR/*.png $pkgdir/share/icons

# base path to source is in parent of SCRIPT_DIR
$bindir/python -m pip install --no-input --no-compile .[all]

cd $tmpdir
# build the client
$bindir/python -m reductus.web_gui.build_client --cleanup

version=$($bindir/python -c "import reductus; print(reductus.__version__)")
