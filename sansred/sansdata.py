...
class SansData(object):
    ...
    def copy(self):
        return SansData(Measurement(self.data.x, self.data.variance),deepcopy(self.metadata),q=copy(self.q),qx=copy(self.qx),qy=copy(self.qy),theta=copy(self.theta), attenuation_corrected=self.attenuation_corrected)
